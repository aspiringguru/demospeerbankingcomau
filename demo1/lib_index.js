...
//NBB: more frams per second = slower.
const FRAMES_PER_PERIOD = 30;

/* eslint-disable-next-line no-unused-vars */
class BarChartRace {
  constructor(container, data) {
    if (!container || !data) throw new Error('Container and data required');
    console.log("class BarChartRace initiated.");
    //console.log("data:"+data);
    console.log("data.length:", data.length);
    console.log("data[0]:"+data[0]);
    console.log("Object.keys(data[0]):"+Object.keys(data[0]));
    //console.log("data[0]['year']:"+data[0]['year']+", data[0]['entries']:"+data[0]['entries']);
    console.log("data[0]['entries'].length:", data[0]['entries'].length);
    console.log("Object.keys(data[0]['entries'][0]):"+Object.keys(data[0]['entries'][0]));
    for (var i = 0; i < data[0]['entries'].length; i++) {
        console.log("data[0]['entries']["+i+"]['name']=", data[0]['entries'][i]['name']);
        console.log("data[0]['entries']["+i+"]['code']=", data[0]['entries'][i]['code']);
        console.log("data[0]['entries']["+i+"]['value']=", data[0]['entries'][i]['value']);
    }
  ...
  resize() {
    ...
    //console.log("in resize(): containerRect = "+containerRect)
  ...
  _drawBar(name, value, idx, maxValue, color) {
    console.log("_drawBar:name:"+name+", \nvalue:"+value+", \nidx:"+idx+", maxValue:"+maxValue);
    ...
    // THRESHOLD : when a bar reaches 90% (THRESHOLD) of its top sibling,
    ...
    console.log("in _drawBar: _color.value="+_color.value+", _color.inverted="+_color.inverted);
    // TODO: add value formatting

    this.ctx.font = `${this.barFontSize}px Arial`;
    console.log("in _drawBar: width="+width)
    console.log("this.barFontSize="+this.barFontSize)
    console.log("this.barHeight="+this.barHeight)
    var barLabel = name.slice(0, (width - 20) / this.barFontSize * 2)
    console.log("in _drawBar: name='"+name+"' length="+name.length)
    console.log("in _drawBar: barLabel="+barLabel)
    if (this.barFontSize*2 > width-20) {
        console.log("in _drawBar: this.barFontSize*2 > width-20")
        //make text same color as number and position to right of the bar
        this.ctx.fillStyle = 'black';
        this.ctx.fillText(name, width + 5, y + this.barHeight*0.9 - 10);
    } else {
        this.ctx.fillText(name.slice(0, (width - 20) / this.barFontSize * 2), this.barFontSize/4, y + this.barHeight / 2 + 5);
    }
    this.ctx.fillStyle = 'black';
    console.log("in _drawBar: Math.trunc(value)="+Math.trunc(value))
  ...
  _drawDateLabel() {
    ...
    const xOffset = dateLabelFontSize * 4.5;
    //this needs adjustment with the range of values.
    const yOffSet = dateLabelFontSize * 0.25;

    this.ctx.font = `${dateLabelFontSize}px Arial`;
    this.ctx.fillStyle = 'grey';
    console.log("in _drawDateLabel: this.fieldWidth:"+this.fieldWidth)
    console.log("in _drawDateLabel: xOffset:"+xOffset)
    console.log("this.data[this.pos].year:"+this.data[this.pos].year)
